...
async function writeTranslationsToDisk(translationMap) {
    for (const toLanguageId in supportedLanguages) {
        const fromLanguageId = supportedLanguages[toLanguageId];
        if (fromLanguageId) {
...
async function collectTranslationsFromRepos(repos, branchname) {
    ...
    for (const repo of repos) {
        ...
        if (!oldBranch) {await gitHelper.push(repo, localRepoPath, branchname);}
        await extractTranslations(localRepoPath + "/" + repo, (componentId, key, languageId, value) => {
            ...
            const existingValue = keyTranslations[languageId];
            if (existingValue) {
...
async function extractTranslations(path, registerTranslation) {
    const ignoreList = [".git", ".circleci", "test", "usecases", "wiki"];
    ...
    fromDir(path,
        ...
        (filename) => {
            ...
            const languageId = filename.substring(filename.length - extension.length - 3, filename.length - extension.length - 1);
        ...
        (filename) => {
            ...
            const languageId = filename.substring(filename.lastIndexOf("/") + 1, filename.lastIndexOf("."));
            const componentId = filename.substring(path.length + 1, filename.lastIndexOf("/i18n"));// .replace(/\//g, ".");
