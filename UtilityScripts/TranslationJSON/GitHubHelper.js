...
async function run() {
    const repos = await iterateRepos([["andariel", "microservice"]])
...
async function branch(reponame, localPath, branchname) {
    ...
    return fileExists(repoPath).
        then((exists) => {
            ...
            const msg = repoPath + " doesnt exist";
            console.log(msg);
...
async function createPR(reponame, branchname) {
    const prName = "i18n-update-" + new Date().toISOString();
...
async function add(reponame, localPath, file) {
    ...
    const output = await exec("cd " + localPath + "/" + reponame + "; git add " + addcmd);
...
async function commit(reponame, localPath, msg) {
    ...
    const output = await exec("cd " + localPath + "/" + reponame + "; git commit -am \"" + msg + "\"");
...
async function clone(reponame, localPath) {
    ...
    try {
        ...
        const output = await exec("git clone ssh://git@github.com/OpusCapita/" + reponame + " " + localPath + "/" + reponame);
...
async function checkout(reponame, localPath, branchname) {
    ...
    const output = await exec("cd " + localPath + "/" + reponame + "; git checkout " + branchname);
...
async function pull(reponame, localPath) {
    ...
    const output = await exec("cd " + localPath + "/" + reponame + "; git pull");
    console.log("pull done, output=", output);
}

async function push(reponame, localPath, branchname) {
    let setupstream = "";
    if (branchname) {setupstream = " --set-upstream origin " + branchname;}
    console.log("pushing repo " + localPath + "/" + reponame);
    const output = await exec("cd " + localPath + "/" + reponame + "; git push" + setupstream);
...
async function iterateRepos(topics) {
    ...
    const url = "https://api.github.com/orgs/OpusCapita/repos";
    let nextPageUrl = url;
    const repos = [];
    ...
    while (true) {
        ...
        const nextPage = res.header["link"].split(",").find((elem) => {return elem.indexOf("rel=\"next\"") > -1});
