...
async function run(srcPath, destPath) {
    ...
    const srcTranslations = require("" + srcPath);
    const srcFolder = srcPath.substring(0, srcPath.lastIndexOf('/'));
    const srcLanguage = srcPath.substring(srcPath.lastIndexOf('/') + 1, srcPath.lastIndexOf('.'));
    console.log("componentFolder=" + srcFolder);
    console.log("srcLanguage=" + srcLanguage);
    const flatdata = await flattenObject(srcTranslations);
